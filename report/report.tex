...
the serve path and as a decay term in eviction. This version replaces an earlier
oracle-cluster, exact-match harness with real embedder-based clustering and a real
semantic index, and that change reverses the report's central finding: under real
clustering, the adjusted Rand index between the k-means-on-embeddings assignment and the
workload's true clusters is 0.02--0.06, barely above chance, because the workload's
canonical query text differs across clusters only in two embedded numbers, and a
general-purpose embedder cannot recover cluster identity from that. Per-cluster
staleness fitting was therefore never actually exercised; ``learned tracks global, not
oracle'' is a diagnosed measurement failure, not a verdict on per-cluster learning in
general. What still holds: the validity gate cuts stale-hit-rate roughly tenfold
regardless of clustering quality, since it applies the same (possibly wrong) cluster
consistently at calibration and serve time; and a fair test of cost-aware eviction, run
for the first time under conditions where eviction actually occurs, shows FreCoS beating
a frequency floor on cost saved with a large, significant effect, though plain LRU beats
FreCoS on both cost saved and stale-hit-rate on this trace. The same real semantic index
also exposes a false-hit problem the old harness could not see: a real 0.8 cosine
threshold on this workload's near-identical templates serves the wrong answer roughly
97\% of the time a hit is served at all: a limitation of this synthetic workload's
text, not of the cache mechanism.
\end{abstract}
...
a real semantic cache than an oracle-cluster, exact-match one. Second, a diagnosed
measurement failure rather than a negative result: cluster identity cannot be recovered
from this workload's query text by a general-purpose embedder, so per-cluster fitting
was never actually exercised under real clustering, and I trace the exact cause (the
query template differs across clusters only in two embedded numbers) rather than
reporting the untestable comparison as if it settled the question. Third, a positive
result on cost-aware eviction: given a fair test under conditions where eviction
actually happens, which every earlier version of this suite failed to arrange, FreCoS's
eviction term measurably beats a frequency floor on cost saved. Section~2 justifies the
baseline, Section~3 describes the design, Section~4 covers the workload and harness, and
Section~5 reports seven experiments: one bracketing run and its two misspecification
follow-ups, one five-row ablation, three parameter sweeps, and one gate-off cost-aware
eviction test.
...
ground truth (Section~3.1). Neither is free of new limitations: a real semantic index
makes false-hit-rate a live metric for the first time, and it is stark: this
workload's canonical query template differs across clusters only in two embedded
numbers, which a real embedder scores at 0.6--0.9 cosine similarity regardless of true
identity, above the 0.8 threshold, so roughly 97\% of served hits across every
bracket-style experiment are false. This is a property of this synthetic workload's
text, not the cache mechanism, but every hit-rate number below should be read alongside
its false-hit-rate (Section~\ref{sec:brackets} carries both). Real clustering similarly
changes every stale-hit-rate number relative to any earlier version of this report.
...
false-hit-rate is large, since it can subtract more than $n_\text{hits}$ and go negative;
per-hit is also the more honest question: given a hit happened, was it worth
anything). Cost saved sums regeneration cost over non-stale hits only, and does not also
exclude false hits, so at this workload's false-hit-rate (roughly 92--97\%, Section~4)
the reported cost-saved numbers are dominated by hits that returned the wrong answer;
\texttt{cost\_saved\_usd} should be read as an upper bound, not money actually saved by
correct answers, until the metric excludes false hits too and every affected experiment
is rerun. Adjusted Rand index (\texttt{cluster\_ari}), reported alongside every
bracket-style result, measures how well the learned assignment matches the true cluster
labels: 1.0 is a perfect match, 0.0 is what a random assignment produces in expectation.
...
oracle, for 10 seeds each on the same 12,000-query W1 configuration, gate enabled at
...
fitting claim this experiment was designed to test. cluster\_ari is 0.036 across every
row, barely above what a random assignment produces. Median n\_hits out of 7560 scored
queries is 4333.5/4252.0/4294.5 for global/learned/oracle; Mann-Whitney vs.\ learned is
$U=49.0$, $p\approx0.940$ (primary, n.s.) for global and $U=96.0$, $p\approx0.0005$,
adj.\ $p\approx0.0046$, $r=0.78$ for oracle.}
\label{tab:brackets}
...
Learned and global, the primary comparison, are statistically indistinguishable
($U = 49.0$, $p \approx 0.940$, $r = 0.02$): pooling every cluster's observations into a
single rate performs the same as fitting each cluster's own. Learned and oracle, a
secondary comparison, separate significantly with a large effect in the direction that
per-cluster fitting is \emph{worse} than an oracle rate keyed to true cluster identity
($U = 96.0$, $p \approx 0.0005$, adjusted $p \approx 0.0046$, $r = 0.78$), the opposite
of an earlier, oracle-cluster version, where learned tracked oracle almost exactly and
beat global.

The reason is clustering quality, not calibration or the maximum-likelihood fit. Median
cluster\_ari across all 30 runs is 0.036, close to the 0.0 a random assignment produces
and far from a perfect match. Direct inspection explains why: W1's canonical query
template (``What is the current status of topic \{cluster\}-\{answer\}?'') is identical
across every cluster except two embedded numbers. Ten canonical queries drawn from ten
different clusters, embedded with the same ONNX model, score 0.6--0.9 cosine similarity
against each other, often higher than genuine same-cluster paraphrase pairs. A
general-purpose embedder is not sensitive to numeric substitution inside an otherwise
fixed template the way it would need to be for cluster identity to be recoverable at
all. This is a property of how W1's text was generated, not a bug in the k-means
implementation or embedder wiring: k-means converges normally on the embeddings it is
given, and it is the embeddings themselves that carry almost no cluster signal.

Three follow-ups (a scarcer-calibration bracket and two misspecification checks,
Section~\ref{ssec:misspec}) all reproduce the identical pattern: learned tracks global,
not oracle, with cluster\_ari in the same 0.036--0.062 range. This rules out calibration
sample size and half-life-distribution shape as confounds; the clustering step, which
happens before either in the pipeline, is where the per-cluster advantage is lost.
Figure~\ref{fig:brackets} plots stale-hit-rate for all three lambda sources across the
well-specified, calibration-sparse, and adversarial-mixture conditions.
...
assumption mattered, independent of clustering; under real clustering, both instead
reproduce the well-specified bracket's learned-tracks-global pattern.

\begin{table}[H]
\centering
\caption{Both misspecification checks reproduce the well-specified bracket's pattern:
learned tracks global (not significant), not oracle (significant, large effect), with
effect sizes close to the well-specified run. Weibull: learned vs.\ global $U=47.0$,
$p\approx0.82$ (n.s.); learned vs.\ oracle $U=100.0$, $p\approx0.0002$, adj.\
$p\approx0.0023$, $r=0.85$. Mixture: learned vs.\ global $U=44.0$, $p\approx0.65$ (n.s.);
learned vs.\ oracle $U=99.0$, $p\approx0.0002$, adj.\ $p\approx0.0026$, $r=0.83$.
cluster\_ari is 0.04--0.06 for both, matching the well-specified run.}
\label{tab:misspec}
\small
\begin{tabular}{lccccc}
\toprule
...
dominant factor is clustering quality, untouched by how half\_life is drawn, since
cluster identity is determined entirely by the query-text embedding step. This rules
out calibration sample size and model misspecification as confounds for the brackets
finding, confirming clustering as the bottleneck regardless of the ground-truth
half-life distribution.

\begin{figure}[H]
\centering
\includegraphics[width=0.48\textwidth]{../analysis/figures/fig1_brackets.png}
\caption{Stale-hit-rate under global, learned, and oracle lambda sources, at the
original calibration scale and a roughly sevenfold sparser scale, both under real
...
I ran five configurations for 10 seeds each at TTL confidence $c=0.90$: stock LRU and
...
and significant: $U = 0.0$, $p \approx 0.00016$, $r \approx 0.85$, cutting stale-hit-rate
roughly 10-fold. That part of the original finding survives the rerun to real clustering
and a semantic index intact. The gate is a per-entry age check against a per-cluster
TTL, and works correctly whether or not the cluster feeding it is right, because the
same assignment is used consistently at calibration and serve time.

Rows 1--3 tie because, at this configuration's real semantic hit rate (~95\%), only
339--372 misses occur out of 7560 scored queries per run, under the 1650-entry budget,
so eviction never runs. LRU, LFU, and the B\&F substitute all tie because none is ever
called to select a victim, not because they agree on one. Section~\ref{ssec:costaware}
confirms this directly by rerunning this exact gate-off configuration at a cache size
small enough to force real eviction, finding real, significant differences between the
three policies there. Rows 4--5 tie for an unrelated reason: FreCoS's decay term
contributes nothing measurable once the gate is active, discussed below.
...
on top of an already-active gate (row 5 vs.\ row 4) gives $\Delta = 0.000$,
...
Peak RSS is statistically indistinguishable between rows, unsurprising since both run
through the same real index on a shared development machine, not a dedicated
benchmarking host. The mean-latency and p95/p99 gap (7ms to 59ms mean) is large because
the gate converts a large fraction of hits into misses at this real hit rate, and a
simulated miss costs far more than a real cache-side decision. Still
...
x 10 seeds, at a cache size (100 entries) chosen after measuring that the main
ablation's 1650-entry cache never fills under this workload's real hit rate (only
339--474 distinct entries are ever needed with an unlimited cache). At 100 entries,
1300+ misses occur per run, forcing real eviction pressure.

\begin{table}[H]
\centering
\caption{With real eviction pressure, FreCoS beats the LFU floor on cost saved with a
large, significant effect: the first result here where FreCoS's cost-awareness shows
measurable work. LRU strictly dominates FreCoS: more cost saved and a lower
stale-hit-rate, with no per-seed overlap on either metric.}
\label{tab:costaware}
\begin{tabular}{lccc}
\toprule
...
(significant, LRU higher). On stale-hit-rate, LRU is lower with no per-seed overlap
(LRU's worst seed, 0.326, still beats FreCoS's best, 0.460): LRU wins on both metrics.
...
likely to already be stale. LRU gets a freshness benefit for free, no staleness model,
no calibration split, no gate, which is a genuine challenge to this project's premise
that cost- and staleness-awareness are needed at all. What does hold up: FreCoS beats
the pure-frequency LFU floor on cost saved with a large effect, the first experiment
where FreCoS's cost term shows real, significant work. It does not establish that FreCoS
beats LRU, and the report does not claim otherwise.
...
set (990 entries) and flattens completely from there; every point from 990 entries on
produces identical per-seed hit counts. The knee falls at 990 entries, smaller than an
earlier exact-match-index sweep found (1980 entries), since the semantic index's higher
baseline hit rate means fewer distinct entries need to be held to saturate it. This
cache size anchors every other sweep in this report.
...
Section~\ref{ssec:stats}) is zero or indistinguishable from zero through confidence
0.90, small but non-zero at 0.95, and only substantial at 0.99, moving in lockstep with
false-hit-rate. Stale-hit-rate falls 18-fold across the same range.}
\label{tab:ttl}
\footnotesize
\begin{tabular}{lcccc}
\toprule
...
Useful-fraction-of-hits stays at or indistinguishable from zero through 0.90, still tiny
at 0.95, and reaches only 0.068 at 0.99 (0.95 vs.\ 0.99: $U=0.0$, $p\approx0.00013$,
adjusted $p\approx0.0023$, $r\approx0.86$, significant). This is a direct consequence of
false-hit-rate sitting above 0.92 at every confidence level: almost every served hit is
false regardless of TTL strictness, since false hits come from the semantic index's
threshold match, not the gate. Raising confidence still buys a real, large
stale-hit-rate reduction (18-fold), but ``the gate is nearly free,'' an earlier
version's central claim, does not survive a real semantic index: this workload's
false-hit problem dominates correctness at every confidence level.

A third sweep varied cluster count at 5, 10, 20, and 50. hit\_rate and stale\_hit\_rate
remain non-monotone with heavily overlapping CIs, matching an earlier exact-match-era
version. cluster\_ari itself rises steadily with $K$ (0.022 at $K=5$ to 0.055 at $K=50$):
more, smaller ground-truth clusters are marginally easier for k-means-on-embeddings to
separate, though every value tested stays close to what a random assignment produces.
...
\caption{Stale-hit-rate and useful-fraction-of-hits against the gate's target confidence
level. Stale-hit-rate falls 18x; useful-fraction-of-hits stays near zero through 0.95
and only becomes substantial at 0.99, tracking false-hit-rate's decline.}
\label{fig:tradeoff}
\end{minipage}
\end{figure}
...
The central finding, Section~\ref{sec:brackets}, is a diagnosed measurement failure, not
a verdict on per-cluster learning: there were no real clusters for it to exploit, so the
ARI numbers there do not license a general claim. An earlier version reported the
opposite conclusion under oracle-perfect cluster identity; that framing was not wrong
given what it tested, but this remediation shows the question (does per-cluster fitting
recover a rate well, given correct clusters) is not yet answerable end to end, because
the harness meant to supply real clusters cannot do so on this workload's text.

What survives: the validity gate still works correctly whether or not the cluster
...
A related finding shares the same root cause: this workload's construction cannot
support a meaningful correctness evaluation once a real semantic index is used, because
the canonical query template differs across clusters and answers only in two embedded
numbers, which a real embedder cannot use to distinguish them. Roughly 92--97\% of hits
served across every experiment, at every TTL confidence tested, are false hits: the
cache confidently returns a real cached answer to a different question than the one
asked. This is a claim about this workload's text, designed for an oracle-cluster world
that never needed the text to be discriminable, not about semantic caches generally.
Every number in Section~5 should be read next to its false-hit-rate until the text is
redesigned or a real corpus replaces it.
...
component's story is the one I expected: the gate holds up regardless of clustering
quality, the per-cluster learning meant to sharpen it was never actually put to the test
its own harness intended (Section~\ref{sec:discussion}), and the eviction term's
cost-awareness, previously reported as contributing nothing, turns out to do real work
once tested under conditions where eviction actually happens, though plain LRU still
beats it here for an unmodeled reason (Section~\ref{ssec:costaware}).
...
semantic index, from the same root cause. This fix is fully specified (a distinct topic
phrase per cluster, a distinct entity phrase per answer\_id, replacing the two-number
template in \texttt{workloads/w1\_synthetic/generator.py}'s canonical-text line) but not
yet applied or rerun: the Phase 2 rerun that produced this version's numbers already
took just under 9 hours, and changing the generator would require rerunning every
experiment against new committed CSVs, which is future work, not a result claimed here.
The same applies to excluding false hits from \texttt{cost\_saved\_usd}
(Section~4): the metric change is prepared, not yet rerun. Running Biton and Friedman's
policy and Cortex's LCFU directly would close the comparator gap now that a real index
exists to run them against. Calibrating the generator against a real inference trace
would strengthen external validity, and breaking the circularity between
stale-hit-rate and its generator-supplied ground truth would be the strongest single
improvement here. Dynamic re-clustering, a FreshCache-style probabilistic staleness
budget, byte-budgeted eviction (where the size term removed from
Equation~\ref{eq:value} would have real meaning), and admission control remain future
work.
